...
// shops prop expected as { success, data: [...] }
export default function ServiceDesign({ services: shops }) {
  return (
    <section className="max-w-[1800px] w-full mx-auto px-2 md:px-8 py-12 font-sans">
      <div className="flex items-baseline justify-between mb-8">
        {/* <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">
      ...
      <ol className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {shops?.data?.map((shop) => (
          <ShotCard
            ...
            shot={{
              id: shop._id,
              image: shop.images?.[0] || "", // Safe access with fallback
              images: shop.images || [], // Pass full images array
              title: shop.description ? 
  (shop.description.length > 10 ? shop.description.substring(0, 20) + "..." : shop.description) : 
  "No info",
              name: shop.name, // Add name for backward compatibility
              description: shop.description || "Great food awaits!", // Add description
              user: {
                name: shop.name, // Or get from shop data if available
                avatar: "https://cdn.dribbble.com/users/6567474/avatars/small/b849c692c6c9fc9cfdca178b90e354d2.png?1607746416",
                badge: shop.location?.stallNumber ? `Stall ${shop.location.stallNumber.trim()}` : "", // Use stall number as badge
              },
              likes: 0, // Default since shops don't have likes initially
              views: 0, // Default since shops don't have views initially
              pro: false,
              team: false,
              href: `/shop/view/${shop.slug}`, // Updated route for shops
      ...
      {/* Updated condition for shop data */}
      {shops?.totalPages > shops?.page ? (
        <div className="flex justify-center mt-12">
